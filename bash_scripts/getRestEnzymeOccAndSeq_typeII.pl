...
if($logFile){open FRAG, ">$fragmentFile" or die "Could not open $fragmentFile\n";
			...
			while ($curOcc != -1) {
				...
				if($prevOcc ne "")
				{
					...
					if($allOccrestSiteSecondInFrag =~ /;/) #has at least one occ of 2nd rest site
					{
						...
						print FRAG $k."\t".$chr."\t".($prevOcc+$lengthRestSite)."\t".$curOcc."\t".substr($seq,$prevOcc+$firstOcc-$length,$length)."\t".($prevOcc+$firstOcc-$length)."\t".($prevOcc+$firstOcc)."\t".substr($seq,$prevOcc+$lastOcc+$lengthSecRestSite,$length)."\t".($prevOcc+$lastOcc+$lengthSecRestSite)."\t".($prevOcc+$lastOcc+$lengthSecRestSite+$length)."\t".$allOccrestSiteSecondInFrag."\t".($curOcc-($prevOcc+$lengthRestSite))."\t".$status."\n";
		...
		if($allOccrestSiteSecondInFrag =~ /;/) #has at least one occ of 2nd rest site
                {
                        ...
                        my $firstOcc=$allOccSecRS[0];
                        my $lastOcc=$allOccSecRS[-1];
			#print "chr=".$chr.";"; print "startFrag=".($prevOcc+$lengthRestSite).";"; print "prevOcc=".$prevOcc.";"; print "lengthResSite=".$lengthRestSite.";";
			#print "curOcc (endFrag)".$curOcc.";";print "seq_StartSeg:".substr($seq,$prevOcc+$firstOcc-$length,$length).";";print "start_startSeg=".($prevOcc+$firstOcc-$length).";"; print "end_endSeg=".($prevOcc+$firstOcc)."\n"; print "seq_endSeg:".substr($seq,$prevOcc+$lastOcc+$lengthSecRestSite,$length).";"; print "start_endSeg=".($prevOcc+$lastOcc+$lengthSecRestSite).";"; print "end_endFrag=".($prevOcc+$lastOcc+$lengthSecRestSite+$length)."\n";
			#print "----> ".$k."\t".$chr."\t".($prevOcc+$lengthRestSite)."\t".$curOcc."\t".substr($seq,$prevOcc+$firstOcc-$length,$length)."\t".($prevOcc+$firstOcc-$length)."\t".($prevOcc+$firstOcc)."\t".substr($seq,$prevOcc+$lastOcc+$lengthSecRestSite,$length)."\t".($prevOcc+$lastOcc+$lengthSecRestSite)."\t".($prevOcc+$lastOcc+$lengthSecRestSite+$length)."\t".$allOccrestSiteSecondInFrag."\t".($curOcc-($prevOcc+$lengthRestSite))."\t".$status."\n";
