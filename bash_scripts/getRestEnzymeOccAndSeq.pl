        ...
        if ($prevOcc ne "") {
          ...
          print FRAG join("\t",
                          ($k,$chr,($prevOcc+$lengthRestSite),$curOcc,
                           substr($seq,$prevOcc+$lengthRestSite,$length),
                           ($prevOcc+$lengthRestSite),($prevOcc+$lengthRestSite+$length),
                           substr($seq,$curOcc-$length,$length),($curOcc-$length),
                           $curOcc,$allOccrestSiteSecondInFrag,
  ...
  if ($prevOcc ne "") {
    ...
    print FRAG join("\t",
                    ($k,$chr,($prevOcc+$lengthRestSite),$curOcc,
                     substr($seq,$prevOcc+$lengthRestSite,$length),
                     ($prevOcc+$lengthRestSite),($prevOcc+$lengthRestSite+$length),
                     substr($seq,$curOcc-$length,$length),($curOcc-$length),$curOcc,
...
sub getStatus()
  {
    ...
    if ($restSiteSecond eq "") {
      $startSeqStatus="StartSeqOK";
      $endSeqStatus="EndSeqOK";
      $midFragStatus="midFragIsValid";
      $fragStatus="FragIsValid";
    }
